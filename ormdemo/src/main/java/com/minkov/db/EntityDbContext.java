...
public class EntityDbContext<T> implements DbContext<T> {
    ...
    private static final String NO_PRIMARY_KEY_EXCEPTION = "Class %s does not have a primary key annotation";
    private static final String PRIMARY_KEY_TEMPLATE = "%s %s PRIMARY KEY AUTO_INCREMENT";
    private static final String ADD_COLUMN_TEMPLATE = "ADD COLUMN %s %s";
    private static final String ALTER_TABLE_TEMPLATE = "ALTER TABLE %s %s";
    private static final String SELECT_COLUMN_NAMES_FROM_DATABASE = "SELECT COLUMN_NAME FROM information_schema.COLUMNS" +
            " WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = '%s'";
    private static final String SELECT_TABLE_NAMES_FROM_DATABASE = "SELECT TABLE_NAME FROM information_schema.TABLES " +
            "WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = '%s'";
    ...
    private Field getPrimaryKeyField() {
        return Arrays.stream(klass.getDeclaredFields())
                ...
                .findFirst()
                .orElseThrow(() -> new RuntimeException(String.format(NO_PRIMARY_KEY_EXCEPTION, klass)));
    }

    private boolean checksIfTableExists() throws SQLException {
        String query = String.format(SELECT_TABLE_NAMES_FROM_DATABASE, this.getTableName());
    ...
    private void createTable() throws SQLException {
        ...
        columnsFields
                .forEach(field -> {
                    ...
                    String columnDefinition = String.format("%s %s",
                            columnName,
                            columnType);
    ...
    private void updateTable() throws SQLException {
        ...
        List<Field> newFields = this.getColumnFields()
                .stream()
                .filter(field -> newColumnsNames.contains(field.getDeclaredAnnotation(Column.class).name()))
                .collect(Collectors.toList());

        List<String> columnsDefinitions = new ArrayList<>();

        newFields.forEach(field -> {
            String columnDefinition = String.format(ADD_COLUMN_TEMPLATE,
    ...
    private List<String> getDatabaseTableColumnsNames() throws SQLException {
        String query = String.format(SELECT_COLUMN_NAMES_FROM_DATABASE, this.getTableName());
